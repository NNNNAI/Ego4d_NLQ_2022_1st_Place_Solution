...
def dataset_gen(
    ...
    def worker(
        ...
        for record in tqdm(worker_data, total=len(worker_data), desc=description):
            ...
            # assert s_ind < 128 and e_ind < 128,'fuck exceed 128'
...
def dataset_gen_bert(data, vfeat_lens, max_pos_len, scope, num_workers=1):
    # Worker method for multiprocessing.
    def worker(
        worker_data, vfeat_lens, max_pos_len, scope, worker_id, output_q
    ):
        worker_dataset = list()
        description = f"process {scope} data [{worker_id}]"
        for record in tqdm(worker_data, total=len(worker_data), desc=description):
...
def gen_or_load_dataset(configs):
    ...
    feature_dir = os.path.join(configs.vslnet_datapath, "features", configs.task, configs.fv)
    ...
    save_path = os.path.join(
        configs.save_dir,
        "_".join(
            [configs.task, configs.fv, str(configs.max_pos_len),str(configs.thres_in_train)]
        )
        + ".pkl",
    )
